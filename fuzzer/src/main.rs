...
impl Window{
    ...
    fn attach(title: &str) -> io::Result<Self>{
        ...
        if ret !=0 {
            ...
            return Ok(Window {
                hwnd: ret,
                seed: Cell::new(0x1337133713371337),
                interesting_keys,
...
fn main() -> io::Result<()> {
    ...
    loop {
        ...
        if unsafe { GetForegroundWindow() } != window.hwnd{ continue; }

        let sel = window.rand() %3;
        match sel {
            0 => {
                window.alt_press(window.rand() as u8 as u16)?;
            }
            1 => {
                window.ctrl_press(window.rand() as u8 as u16)?;
            }
            2 => {
                window.press(window.rand() as u8 as u16)?;
            }
            _ => panic!("Unreachable");
        }

        window.alt_press(b'3' as u16)?;
        return Ok(());
    }
    Ok(())
 }
